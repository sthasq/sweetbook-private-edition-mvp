...
## 현재 심사 기준 상태 (2026-04-15)

- reviewer-facing 기준선은 `심사자 데모 UX와 실연동 신뢰성을 정리한다` PR 계열 상태입니다.
- README는 심사자가 빠르게 팬 주문 동선과 관리자 웹훅 동선을 이해하는 용도로만 유지합니다.
- 심사자용 운영 데이터 목표는 팬 프로젝트 `1~2건`, 주문 `1건`, 진행 중 제작 `1건`, 관리자 웹훅 `1~2건`입니다.
- 운영 데이터가 다시 비대해지면 [`deploy/sql/curate_reviewer_demo_state.sql`](./deploy/sql/curate_reviewer_demo_state.sql)과 [`deploy/README.md`](./deploy/README.md)의 백업 -> 적용 -> 검증 절차로 상태를 되돌립니다.
...
```text
.
├── README.md              # 루트 제품/실행/검증 가이드
├── docker-compose.yml     # 로컬 통합 실행
├── backend/               # Spring Boot API, 도메인, 마이그레이션, 테스트
├── frontend/              # React SPA, 페이지, 공통 컴포넌트, API 클라이언트
├── deploy/                # 운영 배포 스크립트, nginx 설정, reviewer demo SQL
└── .github/workflows/     # CI / 데모 배포 워크플로
```

- 루트는 제품 개요와 실행 방법만 담습니다.
- `backend/`, `frontend/`, `deploy/`는 각 패키지 책임과 운영 스크립트를 가집니다.
- 생성 스크린샷, Stitch 산출물, 회고 같은 참조 자료는 저장소 밖 아카이브로 분리합니다.
