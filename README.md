...
| **크리에이터** | 에디션을 만들고, 공개하고, 팬 주문 상태를 관리 |
...
| 기능 | 설명 |
| --- | --- |
| **대시보드** | 총 매출, 플랫폼 수수료 합계, 크리에이터 정산 합계, 주문 수, 에디션 수, 회원 수를 한눈에 확인 |
| **크리에이터 정산** | 크리에이터별 총 매출·플랫폼 수수료·정산 예정 금액을 조회 |
| **전체 주문 관리** | 크리에이터 관계없이 모든 주문을 조회하고, 각 주문의 수수료 분배와 제작 상태를 확인 |
...
```text
주문 금액 (예: 30,000원)
  ├── 플랫폼 수수료 20% → 6,000원 (PlayPick)
  └── 크리에이터 몫 80% → 24,000원 (크리에이터)
```

수수료율(`commission_rate`), 플랫폼 수수료(`platform_fee`), 크리에이터 몫(`creator_payout`)은 주문 시점에 `customer_order` 테이블에 스냅샷으로 저장됩니다. 이후 수수료율이 변경되더라도 기존 주문의 분배 금액은 보존됩니다.
...
수수료율, 플랫폼 수수료, 크리에이터 몫을 `customer_order` 테이블에 주문 시점 값으로 저장합니다. 런타임 계산 대신 DB 컬럼을 사용한 이유는 (1) 이후 수수료율 변경 시 기존 주문의 정산 금액이 변하지 않도록 보장하고, (2) 관리자 대시보드에서 집계 쿼리 시 별도 계산 없이 `SUM` 한 번으로 정확한 통계를 얻기 위함입니다.
...
| **수수료 관리** | 기본 20% 고정, `application.yml`에서 변경 가능 | 크리에이터별 개별 수수료율 미구현 |
| **환불 처리** | 미구현 | Toss Payments 운영 webhook 범위 밖 |
| **Sweetbook Webhook** | 서명 검증·이벤트 기록 구현 | 운영 환경 실 수신 검증 추가 필요 |
| **YouTube 연동** | 기능 구현 완료 | Google Cloud Console 키 설정 필요 |
