	...
	void assemblesYoutubeRecapIntoSpreadSequence() {
		EditionViews.Detail edition = new EditionViews.Detail(
			...
			new EditionViews.Snapshot(
				...
				Map.of("title", "Official intro", "message", "Creator hello"),
				Map.of("title", "Official closing", "message", "Creator bye"),
		...
		assertThat(preview.pages().get(0).description()).contains("To. 경신");
		assertThat(preview.pages().get(2).description()).contains("1332일");
		assertThat(preview.pages().get(5).title()).isEqualTo("To. 경신");
	}
}
